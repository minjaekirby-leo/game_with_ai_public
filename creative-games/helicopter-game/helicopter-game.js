...
function updateWind() {
    ...
    if (windChangeTimer > 300) { // 5초마다 바람 변경
    ...
    if (Math.abs(windForce) < 0.1) {
        ...
        windText += `오른쪽 ${windForce.toFixed(1)}`;
    } else {
        windText += `왼쪽 ${Math.abs(windForce).toFixed(1)}`;
    }
...
function liftHelicopter() {
    if (gameRunning && !gamePaused && helicopter.fuel > 0) {
        ...
        helicopter.fuel -= helicopter.fuelConsumption * 2; // 상승 시 연료 더 소모
...
canvas.addEventListener('mousedown', liftHelicopter);
